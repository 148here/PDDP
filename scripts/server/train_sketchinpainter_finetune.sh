#!/usr/bin/env bash
set -euo pipefail

PROJECT_ROOT="${PROJECT_ROOT:-/cpfs01/projects-SSD/cfff-27504eab520e_SSD/zwz_42312/yza/PDDP_sketch_inpainting}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/zwz_42312/PDDPoutputs}"
PDDP_CKPT="${PDDP_CKPT:-/home/zwz_42312/temp/downloaded_checkpoints/000297e_1343979iter.pth}"
PYTHON_BIN="${PYTHON_BIN:-/home/zwz_42312/conda_envs/pddp_sketch_inpainting/bin/python}"
RUN_NAME="${RUN_NAME:-sketchinpainter_finetune_bbox_coco_5epoch_v1}"

cd "${PROJECT_ROOT}"
test -f "${PDDP_CKPT}"
mkdir -p "${OUTPUT_ROOT}/train"
export CUDA_VISIBLE_DEVICES=0
"${PYTHON_BIN}" train.py --config_file configs/sketchinpainter_finetune.yaml --name "${RUN_NAME}" --output "${OUTPUT_ROOT}/train" --load_path "${PDDP_CKPT}" --gpu 0 --seed 20260425 --cudnn_deterministic --amp "$@"
